...
class WaterfillingCompression:
    def __init__(self, num_coeffs: int = 4, coeffs_to_keep: str = "optimal"):
        """Computes the waterfilling compression of a dataset. More information about
        this compression algorithm in the paper:
        "Compressive mining:fast and optimal data mining in the compressed domain",
        link to the paper: https://arxiv.org/pdf/1405.5873.pdf


        Args:
            num_coeffs (int, optional): Number of coefficients (of the fourier transform) to
            keep upon compression. Defaults to 4.
            coeffs_to_keep (str, optional): Which coefficient to keep upon compression.
            first: keep only the first `num_coeffs` coefficients of the fourier transform of
            the input.
            best or optimal: keep the largest `num_coeffs` coefficients of the fourier
    ...
    def compute_distance_bounds(self, data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            Tuple[np.ndarray, np.ndarray]: Arrays of shape (n_samples, n_samples),
            upper and lower bound distance matrices respectively.
        """
        (N, dim) = np.shape(data)
        ...
        for i in range(N):
            ...
            if is_even:
                ...
                fx_short = np.concatenate(
                    (fx_short[1:], np.array(fx_short[0]))
                )  # bring DC to the end
            X_f[i] = fx_short
        ...
        for i in range(N):
            for j in range(i + 1, N):
                ub, lb = self.dist_cc(
                    np.conjugate(X_f[i]), np.conjugate(X_f[j]), self.num_coeffs, self.coeffs_to_keep
                )
    ...
    def dist_cc(
        self, x1: np.ndarray, x2: np.ndarray, num_coeffs: int, coeffs_to_keep: str
    ) -> Tuple[float, float]:
        """compute the upper and lower distance bounds between two single data points.


        Args:
            x1 (np.ndarray): Complex conjugate of the fourier transform of the 1st input.
            x2 (np.ndarray): Complex conjugate of the fourier transform of the 2nd input.
            num_coeffs (int): Number of coefficients (of the fourier transform) to keep
            upon compression.
            coeffs_to_keep (str): which coefficients to keep (first, best, optimal).

        Returns:
            Tuple[float, float]: upper and lower bounds on the distance respectively.
        """
        N = len(x1)
        ...
        P1 = np.intersect1d(
            other_coeffs_x1, coeffs_x2
        )  # coefficients where x1_i is unknown and x2_i is known
        P2 = np.intersect1d(
            coeffs_x1, other_coeffs_x2
        )  # coefficients where x1_i is known and x2_i is unknown
        P3 = np.intersect1d(
            other_coeffs_x1, other_coeffs_x2
        )  # coefficients where x1_i and x2_i are unknown

        if coeffs_to_keep == "first":
            # all coefficients are the same
            lb = np.sqrt(2) * np.linalg.norm(
                x1[P0] - x2[P0]
            )  # sqrt(2) needed for storing only one conjugate pair
            ub = lb
        elif coeffs_to_keep == "best":
            lb = np.sqrt(2) * np.sqrt(
                np.linalg.norm(x1[P0] - x2[P0]) ** 2
            ...
            if len(P0) > 0:
                dist = 2 * (
                    np.linalg.norm(x1) ** 2
                    + np.linalg.norm(x2) ** 2
                    - 2 * np.real(np.inner(np.conjugate(x1[P0]), x2[P0]))
                )
            else:
                dist = 2 * (
                    np.linalg.norm(x1) ** 2 + np.linalg.norm(x2) ** 2
                )  # 2 * needed for fourier
            x1_max = np.min(power_x1[coeffs_x1])
            x2_max = np.min(power_x2[coeffs_x2])
            # Double waterfilling
            res = self.double_waterfill(
                power_x1[P2], power_x2[P1], e_x1, e_x2, x1_max, x2_max, P1, P2, P3
            )
